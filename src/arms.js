...
// Compact hero arms: keep the wrist on its own side and choose the outward
// elbow, rather than flipping the bend when the hand passes the shoulder.
export function heroArmPose(shoulder,target,side){
 const hand={x:shoulder.x+side*Math.max(-2,(target.x-shoulder.x)*side),y:target.y};
 const a=armPose(shoulder,hand,1,7,7),b=armPose(shoulder,hand,-1,7,7);
 return a.elbow.x*side>=b.elbow.x*side?a:b;
}
